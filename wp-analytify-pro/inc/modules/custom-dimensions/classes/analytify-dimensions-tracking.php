...
Class Analytify_Dimensions_Tracking {
	...
	*/
	public function _hooks() {
		...
		// add_action('wp_head', array( $this, 'track_dimension' ) ); // ! Debugging.

		// For Google Analytics V4.
		if ( method_exists( 'WPANALYTIFY_Utils', 'get_ga_mode' ) && 'ga4' === WPANALYTIFY_Utils::get_ga_mode() && 'gtag' === ANALYTIFY_TRACKING_MODE ) {
			add_filter( 'analytify_gtag_configuration', array( $this, 'add_dimension_configuration' ) );
			return;
		} 
	...
	/**
	 * Add dimensions data in gtag configuration.
	 *
	 * @param array $configuration
	 * 
	 * @return array
	 */
	public function add_dimension_configuration( $configuration ) {
		...
		foreach ( $current_dimensions as $key => $value ) {
			...
			if ( $identifier && ! empty( $tracking_val ) ) {
	...
	*/
	public function track_dimension() {
		...
		$current_dimensions = $GLOBALS['WP_ANALYTIFY']->settings->get_option( 'analytiy_custom_dimensions','wp-analytify-custom-dimensions' );
		$dimensions = array();
		// get the defined dimensions to set the id if not set.
		$defined_dimensions = Analytify_Google_Dimensions::get_current_dimensions();
		...
		foreach ( $current_dimensions as $key => $value ) {
			$type = $value['type'];
			$id   = $value['id'] ?? $defined_dimensions[$type]['id'];
