...
class RpcServer extends \Grpc\Server
{
    ...
    public function run()
    {
        ...
        while (\true) {
            try {
                ...
                $context = new \Grpc\ServerContext($event);
                $server_writer = new \Grpc\ServerCallWriter($event->call, $context);
                ...
                $server_reader = new \Grpc\ServerCallReader($event->call, $method_desc->request_type);
                try {
                    $this->processCall($method_desc, $server_reader, $server_writer, $context);
                } catch (\Exception $e) {
                    $context->setStatus(\Grpc\Status::status(STATUS_INTERNAL, $e->getMessage()));
                    $server_writer->finish();
                }
            } catch (\Exception $e) {
    ...
    private function processCall(\Grpc\MethodDescriptor $method_desc, \Grpc\ServerCallReader $server_reader, \Grpc\ServerCallWriter $server_writer, \Grpc\ServerContext $context)
    {
        // Dispatch to actual server logic
        switch ($method_desc->call_type) {
            case \Grpc\MethodDescriptor::UNARY_CALL:
                ...
                break;
            case \Grpc\MethodDescriptor::SERVER_STREAMING_CALL:
                $request = $server_reader->read();
