...
namespace Analytify\GuzzleHttp\Handler;

use Analytify\GuzzleHttp\Exception\ConnectException;
use Analytify\GuzzleHttp\Exception\RequestException;
use Analytify\GuzzleHttp\Promise as P;
use Analytify\GuzzleHttp\Promise\FulfilledPromise;
use Analytify\GuzzleHttp\Promise\PromiseInterface;
use Analytify\GuzzleHttp\Psr7\LazyOpenStream;
use Analytify\GuzzleHttp\TransferStats;
use Analytify\GuzzleHttp\Utils;
...
class CurlFactory implements CurlFactoryInterface
{
    ...
    private function applyHandlerOptions(EasyHandle $easy, array &$conf) : void
    {
        ...
        if (!isset($options['sink'])) {
            ...
            $options['sink'] = \Analytify\GuzzleHttp\Psr7\Utils::tryFopen('php://temp', 'w+');
        }
        $sink = $options['sink'];
        if (!\is_string($sink)) {
            $sink = \Analytify\GuzzleHttp\Psr7\Utils::streamFor($sink);
