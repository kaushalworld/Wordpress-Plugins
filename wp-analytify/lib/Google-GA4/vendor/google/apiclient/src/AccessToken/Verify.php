...
use Analytify\Firebase\JWT\ExpiredException as ExpiredExceptionV3;
use Analytify\Firebase\JWT\SignatureInvalidException;
use Analytify\GuzzleHttp\Client;
use Analytify\GuzzleHttp\ClientInterface;
use Analytify\phpseclib3\Crypt\PublicKeyLoader;
use Analytify\phpseclib3\Crypt\RSA\PublicKey;
use Analytify\Psr\Cache\CacheItemPoolInterface;
use Google\Auth\Cache\MemoryCacheItemPool;
use Google\Exception as GoogleException;
use Analytify\Stash\Driver\FileSystem;
use Analytify\Stash\Pool;
use DateTime;
use DomainException;
use Exception;
use Analytify\ExpiredException;
// Firebase v2
use LogicException;
/**
...
class Verify
{
    ...
    {
        $jwtClass = 'JWT';
        if (\class_exists('Analytify\\Firebase\\JWT\\JWT')) {
            $jwtClass = 'Analytify\\Firebase\\JWT\\JWT';
        }
    ...
    private function getPublicKey($cert)
    {
        ...
        if (\class_exists('Analytify\\phpseclib3\\Crypt\\RSA\\PublicKey')) {
    ...
    {
        if (\class_exists('Analytify\\phpseclib3\\Crypt\\RSA')) {
            return 'Analytify\\phpseclib3\\Crypt\\RSA';
        }
        if (\class_exists('Analytify\\phpseclib\\Crypt\\RSA')) {
            return 'Analytify\\phpseclib\\Crypt\\RSA';
        }
        return 'Crypt_RSA';
    }
    private function getBigIntClass()
    {
        if (\class_exists('Analytify\\phpseclib3\\Math\\BigInteger')) {
            return 'Analytify\\phpseclib3\\Math\\BigInteger';
        }
        if (\class_exists('Analytify\\phpseclib\\Math\\BigInteger')) {
            return 'Analytify\\phpseclib\\Math\\BigInteger';
        }
        return 'Math_BigInteger';
    }
    private function getOpenSslConstant()
    {
        if (\class_exists('Analytify\\phpseclib3\\Crypt\\AES')) {
            return 'Analytify\\phpseclib3\\Crypt\\AES::ENGINE_OPENSSL';
        }
        if (\class_exists('Analytify\\phpseclib\\Crypt\\RSA')) {
            return 'Analytify\\phpseclib\\Crypt\\RSA::MODE_OPENSSL';
        }
        if (\class_exists('Analytify\\Crypt_RSA')) {
