...
if ( ! class_exists( 'WP_Analytify' ) ) {
	...
	class WP_Analytify extends Analytify_General {
		...
		private function includes() {
			...
			include_once ANALYTIFY_PLUGIN_DIR . '/classes/analytify-report-abstract.php';
		...
		public function admin_scripts( $page ) {
			...
			if ( 'toplevel_page_analytify-dashboard' === $page ) {
				...
				if ( class_exists( 'WP_Analytify_Pro_Base' ) && version_compare( ANALYTIFY_PRO_VERSION, '5.0.0' ) >= 0 && ( ! isset( $_GET['show'] ) || 'detail-realtime' !== sanitize_text_field( $_GET['show'] ) ) ) {
		 ...
		 * to latest version on analytfiy 5.0.0
		 * 
		 * @since 5.0.0
		 */
		...
		/**
		 * Display a notice that the stored cache is cleard.
		 * 
		 * @since 5.5
		 */
		public function analytify_cache_clear_notice() {
			...
			$message = 'The statistics cache stored by Analytify is successfully cleared.';
