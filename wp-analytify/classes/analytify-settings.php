...
<?php } elseif ( 'wp-analytify-tracking' === $form['id'] AND isset( $form['accordion'] ) ) {
		...
		public function analytify_delete_cache() {
			...
			if( isset( $_GET['analytify_delete_cache_nonce'] ) && wp_verify_nonce( $_GET['analytify_delete_cache_nonce'], 'analytify_delete_cache' ) ) {
				...
				$wpdb->query( $query );
				status_header( 200 );
				$redirect_url = urldecode( $_GET['referrer_url'] );
				$query = parse_url( $redirect_url, PHP_URL_QUERY );
				if ($query) {
					$redirect_url .= '&cache=cleared';
				} else {
					$redirect_url .= '?cache=cleared';
				}
			...
			} else if ( isset( $_POST['analytify_delete_cache_nonce'] ) && wp_verify_nonce( $_POST['analytify_delete_cache_nonce'], 'analytify_delete_cache' ) ) {
				delete_transient( 'analytify_quota_exception' );
				global $wpdb;
				$esc_key = '%' . $wpdb->esc_like( '_analytify_transient' ) . '%';
				$query   = $wpdb->prepare( "DELETE FROM `{$wpdb->prefix}options` WHERE option_name LIKE %s", $esc_key );
