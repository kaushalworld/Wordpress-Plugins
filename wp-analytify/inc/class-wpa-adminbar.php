		...
		if ( $GLOBALS['WP_ANALYTIFY']->pa_check_roles( $is_access_level ) ) {
			...
			foreach ( $menus as $id => $title ) {
				...
				if ( $id === 'analytify-refresh-stats' ) {
					...
					$referrer_url       = ( isset( $_SERVER['HTTPS'] ) && $_SERVER['HTTPS'] == 'on' ? 'https' : 'http' ) . '://' . $_SERVER['HTTP_HOST'];
					$full_referrer_url  = $referrer_url . $_SERVER['REQUEST_URI'];
						...
						array(
							...
							'href'   => add_query_arg(
								array(
									...
									'referrer_url'                 => urlencode( $full_referrer_url ),
								),
								$url
							),
						)
