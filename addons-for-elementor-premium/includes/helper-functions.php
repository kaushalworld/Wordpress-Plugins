 ...
 */
function lae_get_image_html($image_setting, $image_size_key, $settings, $disable_lazy_load = false) {
    ...
    if (!empty($attachment_id) && in_array($size, $image_sizes)) {
        ...
        if ($disable_lazy_load)
            $image_attrs = array_merge($image_attrs, array('data-no-lazy' => 1, 'loading' => 'eager'));

        $image_html .= wp_get_attachment_image($attachment_id, $size, false, $image_attrs);
    }
    else {
        ...
        if (!empty($image_src)) {
            ...
            if (is_null($custom_dimension))
                $image_html .= sprintf('<img %s src="%s" title="%s" alt="%s"%s />', esc_attr($lazy_load_attr), esc_attr($image_src), get_the_title($attachment_id), lae_get_image_alt($attachment_id), $image_class_html);
            else
                $image_html .= sprintf('<img width="%s" height="%s" %s src="%s" title="%s" alt="%s"%s />', esc_attr($custom_dimension['width']), esc_attr($custom_dimension['height']), esc_attr($lazy_load_attr), esc_attr($image_src), get_the_title($attachment_id), lae_get_image_alt($attachment_id), $image_class_html);
        }
    }
