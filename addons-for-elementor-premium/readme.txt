...
= 6.7.2 =
* Fixed – Error with WooCommerce grid quick view function
...
* Improvement - Switched to a simpler authorization mode in Vimeo Grid using Authenticated Personal Access Token instead of Client ID and Client Secret. If you have Vimeo Grid activated on your site, pls generate Personal Access Token for your application created at https://developer.vimeo.com/apps and provide the same in the Social API tab of plugin settings.
...
* Fixed - The loading indicator for the Load More button displayed during filtering
...
* Added - Ability to choose separate Skin Templates for individual items within a Posts Grid. A single Posts Grid can now feature a disparate set of grid items that utilize individual skins of their choice.
...
= 2.9.9 =
* Fixed - The posts carousel and posts grid can cause errors when the taxonomy terms are missing.
...
* Added - Display images and videos in Instagram Grid given one or more user names or hashtags.
...
* Added - Three skins for the display of Instagram posts.
...
= 2.9.2 =
* Added - Vimeo Video Grid/Gallery widget for Elementor that connects to Vimeo Developer API to display videos automatically from Vimeo.
* Added - Display videos in Vimeo Grid given a User ID, Channel ID, Album ID or a Group ID.
* Added - A large number of options for the display of videos like alphabetical, posted date, modified date, view count, number of likes, comments number, and video duration.
...
= 2.9.1 =
* Added - YouTube Video Grid/Gallery widget for Elementor that connects to YouTube Developer API to display videos automatically from YouTube.
...
= 2.9 =
* Added - Load on Scroll (Infinite Scroll) for Posts Grid, Posts Block, and Image/Video Gallery Elementor modules.
* Fixed - A short delay in loading the posts and gallery items after the loading indicator has turned off when using the load more option.
* Fixed - The HTML5 videos would autoplay in the background under certain conditions.

= 2.8 =
* Added - RTL Support for ALL modules part of Livemesh Addons for Elementor.
* Added - The grid items on all relevant Elementor modules like Posts Grid, Posts Block, Gallery, Testimonials, Services, Pricing Table, Icon List, Clients, Stats, Team Members, FAQ, etc. are now displayed from right to left on RTL sites.
...
* Fixed - Fullscreen on YouTube videos lightbox broken on Safari browser.
* Fixed - Subtitle not displayed for Services Carousel.
* Fixed - Styling for Services Carousel was not taking effect.

= 2.7.5 =
* Added - Support for Related Posts and Current Query in Posts Grid, Posts Carousel, and Posts Block modules. These Elementor modules can now be effectively used in Elementor Theme builder templates.
...
* Fixed - The order of the taxonomy terms in the taxonomy terms selection in query builder not the same as the order specified in taxonomy reordering plugins.
...
* Fixed - The direct link to the lightbox image does not work.
...
* Added - Support for AJAX-based filtering instead of JS filtering for video/image gallery addon when pagination/load more function is chosen for the gallery. No more empty filters since the gallery items are fetched from the server when filters are invoked. When the number of items for a filter exceeds the 'items per page' specified, the filters themselves are paginated, or load more function is invoked depending on user choice. The addon reverts to quicker in memory JS filters when no pagination or load more is involved.
...
* Fixed - The video gallery error when the elementor lightbox option is chosen.
...
* Added - Styling options for the read more links and buttons in posts block module.
* Added - Styling options for load more button in posts grid and posts block modules.
* Added - Responsive pagination controls for the gallery.
...
= 2.3.2 =
* Fixed - Gallery pagination would break when the number of items crosses 140.
* Added - Pagination with dotted navigation for galleries when the number of pages exceeds 5.

= 2.3.1 =
* Fixed - Incompatibility with some themes which load waypoints JS script.

= 2.3 =
* Added - Extensive support for filters and templates to enable users to customize the output generated by the addons. You can now create a template file in the elementor-addons folder in your child theme to customize the addons HTML or use filters to customize output generated by the addons.
...
= 2.1 =
* Fixed - Certain widgets/addons like pricing, carousel, gallery, and services raised errors when the 'Add Item' button was pressed to add items. Error seen with the latest version of Elementor.
* Fixed - Leaving tags empty in the gallery lead to the addition of an empty filter in the filter list
...
* Fixed – Translation issues due to the presence of additional language domains.
...
* Added – Seamless control of the number of columns at all device resolutions for all those elements that involve grid – posts/portfolio grid, posts block, image/video gallery, clients, services, testimonials, team, charts, pricing table, faq, etc.
* Added - Option to preserve shortcodes and HTML tags in the excerpt. The option is disabled by default.

= 1.8.8 =
* Fixed – Pagination and Load More for the gallery would break when special characters are present in the title or description.
* Fixed - Duplicate tags filters generated in the gallery when tags have spaces around them.

= 1.8.7 =
* Fixed - Warning for image slider widget in Elementor 2.0
* Fixed - Grid layout for video gallery can break at times when the user expands an inline video fullscreen
* Tweak - Increased the taxonomy terms limit to 500 to help the taxonomy field display all terms in installations where there are a large number of terms

= 1.8.6 =
* Fixed - Broken wide image support for masonry grid in the media gallery

= 1.8.5 =
* Added - Support for multiple URL formats for YouTube and Vimeo videos in the video gallery
...
* Fixed - Next/Prev navigation for the gallery not working
...
= 1.8.2 =
* Improved – License activation page with a less confusing interface.
* Added - The license code entered is now masked upon activation.

= 1.8.1 =
* Added – Ability to display the description for the image/video element in the gallery and gallery carousel lightbox window.
* Added – The lightbox for posts grid and posts block now displays post summary and a link to the post in the lightbox.
* Added – Option to disable the display of post summary/excerpt in the lightbox window of posts block or posts grid.

= 1.8.0 =
* Added – Fancybox lightbox integration for posts grid, posts block, image gallery, video gallery, and gallery carousel modules.
...
* Added - Ability to specify wide width for images in masonry layout of the gallery.
* Added - Options to enable/disable the display of image/video titles and image/video tags in gallery and gallery carousel addons.
...
= 1.7.2 =
* Fixed - Lightbox conflict with Elementor for the gallery, gallery carousel, posts/portfolio grid, and posts blocks modules.
* Added - Option to choose either Elementor or Magnific Popup as lightbox lib for gallery and gallery carousel addons.
* Added - Made Elementor lightbox lib as default for gallery and gallery carousel addons with a slideshow function.
...
= 1.5 =
* Added – Moved the Posts Grid widget to a new framework based on reusable blocks. Please note that the class names have changed to keep it consistent with the new framework.
* Added – 6 different styles for grid items plus 7 header styles too
* Added – Next Prev pagination options for the Posts Grid widget
* Added – Ability to handle a large number of grid pages by providing dotted navigation
...
= 1.1 =
* Fixed - Plugin animations mixing up with elementor animations making the addons to disappear on the frontend
...
* Fixed - Some addons having a strange grey background in elementor editor
